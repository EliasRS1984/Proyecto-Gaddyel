...
// Componente TarjetaProductoDestacado
const TarjetaProductoDestacado = React.memo(({ producto }) => (
    <ScrollReveal>
        <div className="bg-white p-6 rounded-2xl shadow-xl transition-all hover:scale-105 duration-300 transform border border-gray-100 flex flex-col items-center text-center h-96"> {/* Cambio: h-96 para altura fija */}
            ...
            <p className="text-gray-600 text-sm mb-4 line-clamp-3 overflow-hidden">{producto.descripcion}</p> {/* Cambio: line-clamp-3 para limitar descripción */}
            <NavLink to={`/catalogo/${producto.id}`} className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-4 rounded-full text-lg shadow-lg transition-transform transform hover:scale-105 duration-300">
                Ver Producto
            </NavLink>
        </div>
    </ScrollReveal>
...
const Inicio = () => {
    const [productosDestacados, setProductosDestacados] = useState(productos?.filter(producto => producto.destacado) || []);

    useEffect(() => {
        setProductosDestacados(productos?.filter(producto => producto.destacado) || []);
    }, [productos]);
    ...
    return (
        <>
            ...
            <div
                className="w-full min-h-screen bg-fixed bg-cover bg-center md:bg-fixed sm:bg-scroll"
                    ...
                    <div className="flex flex-col items-center w-full">
                        ...
                        <ScrollReveal>
                            {imagenesCarrusel?.length > 0 ? (
                                ...
                                <p className="text-gray-600 text-center">No hay imágenes para mostrar.</p>
                            )}
                        </ScrollReveal>
                    </div>
                </section>
